/**
 * Deploy my-first-contract contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
// membership-club contract has an empty private state ({}): all member data
// flows through the balanceOf witness at proof time, never into storage.
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy membership-club to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState (empty here — the membership-club contract keeps an
      // empty private state). args is the contract constructor's arguments:
      // empty for the no-arg constructor. (Statically-typed contracts can omit
